  ...
  module Role
    ...
    def has_role?(role_name, resource = nil)
      ...
      if new_record?
        ...
        role_array = self.class.role_adapter.where(self.roles, name: role_name, resource: resource)
      end
    ...
    def has_strict_role?(role_name, resource)
      self.class.role_adapter.where_strict(self.roles, name: role_name, resource: resource).any?
    end
    ...
    def has_cached_role?(role_name, resource = nil)
      ...
      self.class.role_adapter.find_cached(self.roles, name: role_name, resource: resource).any?
    end

    def has_strict_cached_role?(role_name, resource = nil)
      self.class.role_adapter.find_cached_strict(self.roles, name: role_name, resource: resource).any?
    end

    def has_all_roles?(*args)
      args.each do |arg|
        if arg.is_a? Hash
          return false unless self.has_role?(arg[:name], arg[:resource])
        elsif arg.is_a?(String) || arg.is_a?(Symbol)
          return false unless self.has_role?(arg)
        else
    ...
    def only_has_role?(role_name, resource = nil)
      self.has_role?(role_name,resource) && self.roles.count == 1
    end
